        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {TESTIMONIALS.map((review, i) => (
            <motion.article
              key={review.name}
              {...entrance(i * 0.12)}
              className="group relative bg-card border border-border hover:border-secondary/30 rounded-3xl p-8 flex flex-col gap-5 shadow-sm hover:shadow-lg transition-shadow transition-colors duration-300"
              aria-label={`Avaliação de ${review.name}`}
            >
              {/* Quote icon */}
              <Quote
                className="w-8 h-8 text-secondary/25 group-hover:text-secondary/40 transition-colors duration-200 flex-shrink-0"
                aria-hidden="true"
              />
